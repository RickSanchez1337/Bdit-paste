...
std::wstring read_unicode(const std::uintptr_t address, std::size_t size)
{
	...
	mex.ReadString(address, buffer.get(), size * 2);
...
std::string read_ascii(const std::uintptr_t address, std::size_t size)
{
	...
	mex.ReadString(address, buffer.get(), size);
...
public:
	...
	std::wstring GetItemName()
	{
		...
		auto DisplayName = mex.Read<uintptr_t>(Info + 0x28);
		//auto name = mex.Read<pMyUncStr>(DisplayName + 0x18);
		auto wname = read_unicode(DisplayName + 0x18, 0x64);
		if (!wname.empty())
		{
			return wname;
		}
...
public:
	Item GetItem(int id)
	{
		...
		item.Item = TheItem;
		mex.ReadString(item.Item, &item.buffer, sizeof(item.buffer));
		return item;
	}
};
...
public:
	ItemContainer GetBelt()
	{
		...
		mex.ReadString(ic.ItemContainer, &ic.buffer, sizeof(ic.buffer));
		return ic;
	}
};
...
public:
	...
	PlayerModel GetPlayerModel()
	{
		...
		mex.ReadString(model.Model, &model.buffer, sizeof(model.buffer));
		return model;
	}
	...
	PlayerInventory GetPlayerInventory()
	{
		...
		mex.ReadString(pi.Inventory, &pi.buffer, sizeof(pi.buffer));
		return pi;
	}
